...
class useradmin_users extends table_desc{
	protected function init($params){
		...
		$this->options['list'] = array(
			...
			'nickname',
			'email',
			'role_id'=>array('label'=>'Role', 'search'=>true, 'editable'=>true, 'editrules'=>array('required'=>true)),
			'groups_id'=>array('label'=>'Group', 'search'=>true, 'editable'=>true, 'editrules'=>array('required'=>true)),
			'company_id',
			'status_id'
        );
		$this->options['edit'] = array('company_id', 'username', 'nickname', 'email', 'role_id', 'groups_id'); 
		$this->options['linkTables'] = array('m2m'=>array(
			'groups', 
			'role'=>array('link_table'=>'role_user', 'self_link_field'=>'user_id'))
			);
	}
	
	protected function _getLimit($params){ //只能看自己公司的用户
		$ret = array();
		$ids = array();
		$res = $this->tool->query("SELECT id from users where company_id={$this->userInfo->company_id}");
		while($row = $res->fetch())
			$ids[] = $row['id'];
		$ret[] = array('field'=>'useradmin.users.id', 'op'=>'IN', 'value'=>implode(',', $ids));
		return $ret;
	}
