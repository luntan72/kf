...
class xt_zzvw_prj_action_getboardtype extends action_jqgrid{
	protected function handlePost(){
		...
		$where = "board_type.name IS NOT NULL AND group_testcase_type.group_id IN (".$this->userInfo->group_ids.")";
		if (!empty($this->params['os_id']))
			$where .= " AND prj.os_id=".$this->params['os_id'];
		if (!empty($this->params['chip_id']))
			$where .= " AND prj.chip_id=".$this->params['chip_id'];
