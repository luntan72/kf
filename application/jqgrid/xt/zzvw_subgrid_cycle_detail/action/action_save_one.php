...
class xt_zzvw_subgrid_cycle_detail_action_save_one extends xt_zzvw_cycle_detail_action_save_one{
	...
	protected function returnData($data){
		...
		$total_res = $this->tool->query("SELECT id, comment, result_type_id, issue_comment, test_env_id, defect_ids, jira_key_ids, finish_time FROM cycle_detail WHERE id in (".implode(',', $element).")");
		$total = array('comment'=>'null', 'issue_comment'=>'null', 'defect_ids'=>'null');
		while($row = $total_res->fetch()){
			if(!empty($row['defect_ids']) && $row['defect_ids'] != '')
				$total['defect_idss'][] = $row['defect_ids'];
			if(!empty($row['jira_key_ids']) && $row['jira_key_ids'] != '')
				$total['jira_key_idss'][] = $row['jira_key_ids'];
			$total['result_type_id'][] = $row['result_type_id'];
			$total['comments'][] = $row['comment'];
			$total['issue_comments'][] = $row['issue_comment'];
			$total['finish_times'][] = $row['finish_time'];
		}
		...
		if(!empty($total['jira_key_idss'])){
			$total['jira_key_idss'] = array_unique($total['jira_key_idss']);
			if(count($total['jira_key_idss']) == 1){
				if(!empty($total['jira_key_idss'][0]))
					$total['jira_key_ids'] = $total['jira_key_idss'][0];
			}
			else
				$total['jira_key_ids'] = implode(',', $total['jira_key_idss']);
			unset($total['jira_key_idss']);
		}
		
		if(!empty($total['finish_times'])){
			$total['finish_times'] = array_unique($total['finish_times']);
			if(count($total['finish_times']) == 1){
				if(!empty($total['finish_times'][0]))
					$total['finish_time'] = $total['finish_times'][0];
			}
			else{
				foreach($total['finish_times'] as $key=>$finsh_time){
					if(0 == $key)
						$time = $total['finish_times'][0];
					else if($time < $finsh_time){
						$time = $finsh_time;
					}
				}
				$total['finish_time'] = $time;
			}
			unset($total['finish_times']);
		}
