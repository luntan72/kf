...
class action_ver_link2prj extends action_ver_action{
	...
	*/
	protected function handlePost(){
		...
		$sql = "SELECT {$this->link_table}.* FROM {$this->link_table} ".
			" left join {$this->ver_table} on {$this->link_table}.$ver_id={$this->ver_table}.id".
			" WHERE prj_id={$params['prj_id']} AND {$this->link_table}.$table_id IN ($t_ids) AND {$this->ver_table}.edit_status_id IN ($edit_statuses)";
		$res = $this->tool->query($sql);
		while($row = $res->fetch()){
			$tmp = $this->tool->query("SELECT group_concat(distinct {$this->link_field}) as prj_ids ".
				" from {$this->link_table} left join {$this->ver_table} on {$this->link_table}.$ver_id={$this->ver_table}.id ".
				" where $ver_id={$row[$ver_id]} AND {$this->ver_table}.edit_status_id IN ($edit_statuses)");
			$tmpRow = $tmp->fetch();
