...
(function(){
	...
	DB.dingdan_cg = function(grid){
		$table.supr.call(this, grid);
	};

	var $table = DB.dingdan_cg;
			...
			case 'jh': //重启订单
				if (XT.checkSelectedRows(selectedRows, 1)){
					...
					var buttons = {
						'重启订单':function(){
							var dialog = this;
							$.post('/jqgrid/jqgrid/oper/change_status/db/qygl/table/dingdan_cg', {id:element, status:'jh'}, function(data){
	// XT.debug(data);
								$(dialog).dialog('close');
				...
				if (XT.checkSelectedRows(selectedRows, 1)){
					...
					var buttons = {
						'结束订单':function(){
							var dialog = this;
							XT.debug(dialog);
							$.post('/jqgrid/jqgrid/oper/change_status/db/qygl/table/dingdan_cg', {id:element, status:'jieshu'}, function(data){
	// XT.debug(data);
								$(dialog).dialog('close');
								$(gridId).trigger('reloadGrid');
							}, 'json');
